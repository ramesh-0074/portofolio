...
function Card({ title, index, url, description, thumbnail, stack }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isMobile) {
      // For non-mobile devices, apply slide-in animation
  ...
  return (
    <motion.div
      ref={cardRef}
      className={`relative w-full rounded-2xl md:w-[45vw] h-auto min-h-[50vh] flex flex-col items-center justify-center border border-gray-300 px-3 py-4 my-5 shadow-md ${
        index % 2 === 0
    ...
    >
      ...
      <p ref={descriptionRef} className="text-[#C1E8FF] dark:text-[#1c2e4a] text-center mt-2 opacity-0">
        {description}
      </p>
      <div
        ref={stackRef}
      ...
      <Link
        ...
        href={url}
        rel="noopener noreferrer"
        className="absolute bottom-1 text-[#C1E8FF] dark:text-[#1c2e4a] flex items-center gap-1 font-semibold text-sm hover:underline"
      >
